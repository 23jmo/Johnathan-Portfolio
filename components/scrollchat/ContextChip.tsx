...
export default function ContextChip({ context, onRemove }: ContextChipProps) {
  return (
    <motion.div
      ...
      className="flex items-center gap-1.5 rounded-full border border-foreground/15 bg-foreground/10 py-1 pl-1 pr-2 text-xs text-foreground/80"
    >
      ...
      {onRemove && (
        <button
          ...
          className="rounded-full p-0.5 text-foreground/50 transition-colors hover:bg-foreground/10 hover:text-foreground"
        >
